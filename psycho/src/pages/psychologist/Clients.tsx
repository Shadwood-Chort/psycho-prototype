...
export function Clients() {
  ...
  return (
    <div className="flex min-h-full flex-col">
      <TopBar title={t("Клиенты")} />
      <div className="flex-1 space-y-3 px-5 pb-6">
        {clients.length === 0 && (
          <p className="pt-10 text-center text-[14px] text-sage-500">{t("Пока нет клиентов.")}</p>
        )}
        {clients.map((a) => (
          <Card
            key={a.clientName}
          ...
          >
            ...
            <div>
              ...
              <p className="text-[12px] text-sage-500">
                {a.clientRequestSummary ? a.clientRequestSummary.slice(0, 50) + "…" : t("Нет анкеты")}
              </p>
            </div>
          </Card>
        ))}
